...
def train_one_epoch(
    ...
    for batch in pbar:
        ...
        outputs = model.compute_loss(input_ids, attention_mask, target_ids)
        loss = outputs["loss"]
...
def train_rec(config:dict = CONFIG):
    ...
    for epoch in range(1, config['epochs'] + 1):
        ...
        print(f"\nEpoch: {epoch:3d}/{config['epochs']} lr={current_lr:.2e} loss={train_loss:.4f}")
    ...
    print(f"Loading Best Model (Epoch {ckpt['epoch']}, val NDCG@{monitor_k}={best_val_ndcg:.4f})..")
