...
class SeqTrainDataset(Dataset):
    ...
    input_ids构造:
    [PAD, ..., PAD, BOS, c0_i1,     c1_i1,  c2_i1,   c0_i2, c1_i2, ...]
    ←─ padding ─→       ←─── item_1 的3个token ───→  ←─── item_2 ───→
    ...
    def __init__(self, user_histories: Dict[int, List[int]],    # {user_id:[item_id,...]}
        ...
        for user_id, history in user_histories.items():
            ...
            if len(history) == 0:
                continue
            self.samples.append((history, targets[user_id]))
    ...
    def __getitem__(self, idx:int) -> dict:
        ...
        # 左 Padding
        # 保证最后一个有效的token永远在序列末尾
        # -> 取logits[-3:]永远对应目标item的预测位置
...
def get_rec_loaders(
    ...
    train_ds = SeqTrainDataset(
        user_histories=data['train'],
        targets=data['val'], # 训练时预测val target，训练结束后直接在val上评估
        **common_kwargs
    )
